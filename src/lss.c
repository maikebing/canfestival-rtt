...
UNS8 proceedLSS_Slave(CO_Data *d, Message *m)
{
	...
	case LSS_CONF_NODE_ID: /* Configure Node-ID */
	{
		...
		if (d->lss_transfer.mode == LSS_CONFIGURATION_MODE)
		{
			...
			else
			{
				...
				write_eeprom(NODEID_FLAG, 0x66);
				write_eeprom(NODEID, d->lss_transfer.nodeID);
				setNodeId(&TestSlave_Data, d->lss_transfer.nodeID);
	...
	case LSS_CONF_BIT_TIMING: /* Configure Bit Timing Parameters */
	{
		...
		if (d->lss_transfer.mode == LSS_CONFIGURATION_MODE)
		{
			...
			write_eeprom(BAUDRATE_FLAG, 0x66);
			write_eeprom(BAUDRATE, Buadrate2set);
			can_Init();
			setState(&TestSlave_Data, Initialisation); // Init the state
		}
		else
